...
def secrets_ok() -> tuple[bool, str]:
    ...
    if result.returncode != 0:
        detail = (result.stderr or result.stdout).strip() or "hallazgos detectados"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Inspector determinista de controles de seguridad (GATE-SEC)")
