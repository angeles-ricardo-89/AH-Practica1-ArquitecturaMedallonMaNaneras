...
produccion (OWASP A02, id de catalogo HDRS), condiciones excepcionales y consumo acotado (A10 y
LLM06), logging sin secretos (A09) y cadena de suministro (A03). Tambien cubre el diseno
...
- Catalogo: `governance/security-controls.yaml` (controles HDRS, A10, A09, A03 y SEC).
...
- Prueba: `backend/tests/test_security.py` (marcadores HDRS y A10) + chequeos `lockfiles` y `secrets-scan` del catalogo

## Amenaza y control

- **A02/HDRS Misconfiguration:** headers `X-Content-Type-Options: nosniff`, `Referrer-Policy:
...
- **A10/LLM06 Exceptional Conditions:** body limit 64 KB con `413` en POST/PUT/PATCH;
...
- [ ] Headers presentes en cualquier respuesta; HSTS/CSP solo en prod (HDRS, `tests/test_security.py`: `test_security_headers_always_present`, `test_hsts_and_csp_only_in_production`, `test_no_hsts_csp_in_local`).
- [ ] `/docs` y `/openapi.json` -> `404` en prod (HDRS, `test_docs_disabled_in_production`).
- [ ] CORS: origen no permitido sin `Access-Control-Allow-Origin`; permitido lo recibe exacto (HDRS, `test_cors_exact_origin`, `test_cors_fail_closed_by_default`).
- [ ] Body grande -> `413`; timeouts de BD aplicados (A10, `test_body_size_limit_returns_413`, `test_pg_connect_applies_timeouts`).
- [ ] `make security`: controles HDRS/A10/A09/A03/SEC en estado OK.
