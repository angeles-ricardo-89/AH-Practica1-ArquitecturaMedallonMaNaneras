# Domain Skill: Manejo de Salida del Modelo (LLM10 + LLM08)

## Proposito

Gobernar el tratamiento de la salida del modelo: nunca se ejecuta ni se inyecta HTML crudo
(OWASP LLM10 Improper Output Handling) y los errores internos nunca filtran detalle al cliente
(OWASP LLM08 Hidden Context Exposure). La salida se sanea en el frontend con DOMPurify y el
...
- Spec de seguridad: `docs/superpowers/specs/2026-09-05-security-hardening-gate-design.md` (secciones 5.3, 7.1 y 8.4, controles LLM10/LLM08).
- Catalogo: `governance/security-controls.yaml` (controles LLM10 y LLM08).
...
- **LLM10 Improper Output Handling:** la salida del modelo puede contener HTML o markdown
...
- **LLM08 Hidden Context Exposure:** un error interno puede devolver `str(exc)`, el tipo de la
...
| Testing de Seguridad | `../tech/security_testing.md` | Marcadores LLM10/LLM08 y gate `make security` |
...
- [ ] `<script>`, handlers `onerror` y URLs de esquema `javascript:` quedan eliminadas de la salida saneada (LLM10, marcador `// security: LLM10` en `frontend/tests/utils/markdown.test.ts`; hoy cubre `strips script tags` y `strips img onerror handlers`).
- [ ] Un `RuntimeError` interno devuelve `503` sin el texto de la excepcion (LLM08, `tests/test_security.py`: `test_runtime_error_does_not_leak_internals`).
- [ ] `cd frontend && pnpm test:unit` verde.
- [ ] `make security`: controles LLM10 y LLM08 en estado OK.
